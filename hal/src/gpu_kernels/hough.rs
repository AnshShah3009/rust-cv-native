...
pub fn hough_lines(
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Hough Dispatch") });
    
    // Clear accumulator first
    encoder.clear_buffer(&accumulator_buffer, 0, None);

    {
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Hough Vote"), timestamp_writes: None });
