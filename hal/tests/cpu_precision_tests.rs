...
fn test_convolution_for_type<T: Float + std::ops::Neg<Output = T> + 'static>() {
    ...
    data[0] = T::ONE; data[1] = T::ZERO; data[2] = T::ZERO;
    data[3] = T::ONE; data[4] = T::ZERO; data[5] = T::ZERO;
    data[6] = T::ONE; data[7] = T::ZERO; data[8] = T::ZERO;
    
    let input: Tensor<T, CpuStorage<T>> = Tensor::from_vec(data, cv_core::TensorShape::new(1, 3, 3)).unwrap();
    
    let kernel_data = vec![
        T::ONE, T::ZERO, T::ONE.neg(),
        T::ONE, T::ZERO, T::ONE.neg(),
        T::ONE, T::ZERO, T::ONE.neg(),
    ];
    let kernel: Tensor<T, CpuStorage<T>> = Tensor::from_vec(kernel_data, cv_core::TensorShape::new(1, 3, 3)).unwrap();
