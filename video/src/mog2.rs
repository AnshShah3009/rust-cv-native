    ...
    where Tensor<u8, S>: TensorToCpu<u8> + TensorToGpu<u8>
    {
        ...
        match ctx {
            ComputeDevice::Gpu(gpu) => {
                ...
                let frame_f32 = <Tensor<u8, GpuStorage<u8>> as TensorCast>::to_f32_ctx(&frame_gpu, gpu).expect("GPU cast failed");
                ...
                if self.model.is_none() || self.width != width || self.height != height {
                    ...
                    let model_gpu = CpuTensor::from_vec(data, TensorShape::new(1, 1, width * height * self.n_mixtures * 3))
                        .expect("Failed to create model tensor").to_gpu_ctx(gpu).expect("Upload model to GPU failed");
                    self.model = Some(Box::new(model_gpu));
                }

                let model_gpu = self.model.as_mut().expect("Model missing").downcast_mut::<Tensor<f32, GpuStorage<f32>>>().expect("Downcast failed");
                let mut mask_gpu = Tensor::<u32, GpuStorage<u32>>::from_vec(vec![0u32; width * height], TensorShape::new(1, height, width))
                    .expect("Failed to create mask tensor").to_gpu_ctx(gpu).expect("Upload mask to GPU failed");
                
                gpu.mog2_update(&frame_f32, model_gpu, &mut mask_gpu, &params).expect("MOG2 GPU update failed");
                
                let mask_cpu = mask_gpu.to_cpu_ctx(gpu).expect("Download mask to CPU failed");
                let u8_data: Vec<u8> = mask_cpu.as_slice().expect("Failed to get mask slice").iter().map(|&v| v as u8).collect();
                CpuTensor::from_vec(u8_data, frame.shape).expect("Failed to create final result tensor")
            },
            ComputeDevice::Cpu(cpu) => {
                let frame_cpu = frame.to_cpu().expect("Download frame to CPU failed");
                let frame_f32_vec: Vec<f32> = frame_cpu.as_slice().expect("Failed to get frame slice").iter().map(|&v| v as f32 / 255.0).collect();
                let frame_tensor = CpuTensor::from_vec(frame_f32_vec, frame.shape).expect("Failed to create frame tensor");
                ...
                if self.model.is_none() || self.width != width || self.height != height {
                    ...
                    self.model = Some(Box::new(CpuTensor::from_vec(data, TensorShape::new(1, 1, width * height * self.n_mixtures * 3)).expect("Failed to create model tensor")));
                }

                let model_cpu = self.model.as_mut().expect("Model missing").downcast_mut::<CpuTensor<f32>>().expect("Downcast failed");
                let mut mask_cpu = Tensor::<u32, CpuStorage<u32>>::from_vec(vec![0u32; width * height], frame.shape).expect("Failed to create mask tensor");
                
                cpu.mog2_update(&frame_tensor, model_cpu, &mut mask_cpu, &params).expect("MOG2 CPU update failed");
...
mod tests {
    ...
    fn test_mog2_basic() {
        ...
        let bg_frame: CpuTensor<u8> = CpuTensor::from_vec(bg_data, TensorShape::new(1, height, width)).unwrap();
        ...
        let fg_frame: CpuTensor<u8> = CpuTensor::from_vec(fg_data, TensorShape::new(1, height, width)).unwrap();
