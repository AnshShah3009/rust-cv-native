...
fn detect_with_precision<T: Float + bytemuck::Pod + bytemuck::Zeroable + 'static>(name: &str, cpu: &CpuBackend, threshold: T) {
    let width = 640;
    let height = 480;
    ...
    let tensor: Tensor<T, CpuStorage<T>> = Tensor::from_vec(data, cv_core::TensorShape::new(1, height, width)).unwrap();
    ...
    println!("Found FAST score map of size {} using precision format {}", score_map.shape.len(), name);
}
