...
fn test_haar_scale_factor_impact() {
    ...
    let detections_fast = cascade.detect(&img, 1.5, 0);  // Larger scale factor
    let detections_slow = cascade.detect(&img, 1.05, 0); // Smaller scale factor
...
fn test_haar_rect_sum_correctness() {
    ...
    let left_sum = get_rect_sum(&integral, 0, 0, 4, 8);
    assert_eq!(left_sum, 3200);

    // Sum of right half (4x8, value 200) should be 4 * 8 * 200 = 6400
    let right_sum = get_rect_sum(&integral, 4, 0, 4, 8);
